...
- 위험 수준, 요약 메시지, 즉시 행동 가이드를 짧게 보여준다.
- **상세 리포트 보기 버튼**을 제공해 웹 리포트 페이지로 이동시킨다.
- **경찰 신고 / 신고 안내 버튼**을 제공해 공식 신고 경로로 빠르게 이동시킨다.
...
- 스캔 결과 기반 챗 API와 사용자 피드백 저장 API를 제공한다.
...
6. Backend 내부에서는 다음 순서로 분석한다.
   ...
   - LLM 설명 생성
   - 결과 저장
7. 분석이 끝나면 Extension은 하이라이트, 요약, 상세 리포트 버튼, 신고 버튼을 렌더링한다.
8. 사용자가 상세 보기를 누르면 웹 리포트 페이지로 이동해 더 자세한 결과를 본다.
...
## 메모
- 확장 프로그램은 **빠른 경고와 하이라이트**에 집중하고, 웹 리포트 페이지는 **상세 설명과 시각화**에 집중한다.
...
- 경찰 신고 기능은 초기에는 **자동 제출**이 아니라 **공식 신고 경로로의 연결과 안내** 수준으로 설계한다.
